...
static int mmc3680kj_validate_id(i2c_dev_t* drv, uint8_t product_id)
{
    ...
    ret = sensor_i2c_read(drv, MMC3680KJ_REG_PRODUCTID, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
...
static int mmc3680kj_check_otp(i2c_dev_t* drv)
{
    ...
    ret = sensor_i2c_read(drv, MMC3680KJ_REG_STATUS, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
...
static int mmc3680kj_get_comp_matrix(i2c_dev_t* drv)
{
    ...
    ret = sensor_i2c_write(drv, MMC3680KJ_REG_PASSWORD, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }

    value = MMC3680KJ_CMD_OTP_OPER;
    ret = sensor_i2c_write(drv, MMC3680KJ_REG_OTPMODE, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }

    value = MMC3680KJ_CMD_OTP_MR;
    ret = sensor_i2c_write(drv, MMC3680KJ_REG_TESTMODE, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }

    value = MMC3680KJ_CMD_OTP_ACT;
    ret = sensor_i2c_write(drv, MMC3680KJ_REG_CTRL2, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
...
static int mmc3680kj_set_pulse_width(i2c_dev_t* drv)
{
    ...
    ret = sensor_i2c_read(drv, MMC3680KJ_REG_SR_PWIDTH, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
    ...
    ret = sensor_i2c_write(drv, MMC3680KJ_REG_SR_PWIDTH, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
...
static int mmc3680kj_set(i2c_dev_t* drv)
{
    ...
    ret = sensor_i2c_write(drv, MMC3680KJ_REG_CTRL0, &value, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
...
static int mmc3680kj_set_output_resolution(i2c_dev_t* drv, uint8_t res)
{
    ...
    ret = sensor_i2c_write(drv, MMC3680KJ_REG_CTRL1, &res, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
...
static int drv_mag_memsic_mmc3680kj_read(void *buf, size_t len)
{
    ...
    ret = sensor_i2c_read(&mmc3680kj_ctx, MMC3680KJ_REG_DATA, reg_raw, MMC3680KJ_MAG_DATA_SIZE, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
    ...
    pdata->data[0] = (int32_t)(mag_raw[0] - MMC3680KJ_OFFSET) * g_otp_matrix[0] / MMC3680KJ_SENSITIVITY;
    pdata->data[1] = (int32_t)(mag_raw[1] - MMC3680KJ_OFFSET) * g_otp_matrix[1] / MMC3680KJ_SENSITIVITY;
    pdata->data[2] = (int32_t)(mag_raw[2] - MMC3680KJ_OFFSET) * g_otp_matrix[2] / MMC3680KJ_SENSITIVITY;
...
int drv_mag_memsic_mmc3680kj_init(i2c_dev_t *i2c_dev)
{
    int ret = 0;
    ...
    if (!g_init_bitwise) {
        mmc3680kj_ctx.port = i2c_dev->port;
        mmc3680kj_ctx.priv = i2c_dev->priv;
    ...
    if (!(g_init_bitwise & (1 << FLAG_INIT_MAG))) {
        ...
        ret = mmc3680kj_set_output_resolution(&mmc3680kj_ctx, MMC3680KJ_CMD_100HZ);
        if (unlikely(ret)) {
                return -1;
        }
...
static int drv_temp_memsic_mmc3680kj_read(void *buf, size_t len)
{
    ...
    ret = sensor_i2c_read(&mmc3680kj_ctx, MMC3680KJ_REG_TEMP, &reg_raw, I2C_DATA_LEN, I2C_OP_RETRIES);
    if (unlikely(ret)) {
        return ret;
    }
...
int drv_temp_memsic_mmc3680kj_init(i2c_dev_t *i2c_dev)
{
    int ret = 0;
